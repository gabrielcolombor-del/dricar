...
async function main() {
  console.log("--> Iniciando sincronização instantânea em lote para Supabase...");
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (existingVenda) {
      const formattedDate = dataVendaParsed.toISOString();
      const finalValor = valorVenda > 0 ? valorVenda : parseFloat(veiculo.valorCompra.toString());

      try {
        await prisma.$executeRawUnsafe(
          `UPDATE "vendas" SET "data_venda" = '${formattedDate}'::date, "valor_venda_veiculo" = ${finalValor} WHERE "id" = '${existingVenda.id}'::uuid;`
        );
        await prisma.$executeRawUnsafe(
          `UPDATE "veiculos" SET "data_entrada" = '${formattedDate}'::date, "status" = 'Vendido' WHERE "id" = '${veiculo.id}'::uuid;`
        );
        count++;
      } catch (e) {
        // ignora
  ...
  console.log(`\n⚡ CONCLUÍDO! Total de ${count} vendas e veículos sincronizados com datas de 2020 a 2026!`);
}
