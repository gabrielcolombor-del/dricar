...
export default function CrmTab() {
  ...
  const openRegisterSale = (lead) => {
    ...
    // Pre-povoar valor de venda se veículo tiver valor
    let defaultPrice = "";
    if (lead.veiculoInteresse) {
      defaultPrice = "R$ " + Number(lead.veiculoInteresse.valorCompra * 1.15).toLocaleString("pt-BR", { maximumFractionDigits: 0 }); // Sugere preço com margem
    }
    setFormVenda({
  ...
  return (
    <div className="space-y-6 text-gray-800 animate-fade-in">
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h4 className="font-extrabold text-brand-blue text-sm uppercase tracking-wider">Funil de Vendas e Leads (CRM)</h4>
          <p className="text-xs text-gray-400">Gerencie potenciais compradores e converta-os em vendas integradas ao estoque.</p>
        </div>
        <button
          onClick={() => {
            setFormLead({
                  ...
                  {leadsNaColuna.length === 0 ? (
                    ...
                    leadsNaColuna.map(lead => (
                      <div
                        ...
                        className={`bg-white border border-gray-150 p-3.5 rounded-xl shadow-sm hover:shadow transition-shadow flex flex-col gap-2 relative ${
                      ...
                      >
                        ...
                        {/* Mobile quick navigation / Register Sale button */}
